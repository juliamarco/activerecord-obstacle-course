...
# ========================
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.where(user_id: @user_3.id).third.items
                                        .pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    data = User.joins(:order_items)
               .select("name AS user_name, order_id, orders.amount / COUNT(order_items.id) AS avg_item_cost")
               .group("name", "order_id")
               .order(name: :desc)
    # ---------------------------------------------------------------
